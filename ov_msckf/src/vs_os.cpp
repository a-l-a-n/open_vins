...
int bindCpu(int cpu_id, std::thread *thread_ptr)
{
    ...
    if(thread_ptr) pthread_setaffinity_np(thread_ptr->native_handle(),
                                          sizeof(cpu_set_t), &cpuset);
    else pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);
    return 0;
}
...
std::string abspath(const char* path)
{
    ...
    if(realpath(path, abs_path)) return std::string(abs_path);
...
std::vector<std::string> listdir(const char* path, bool absname, bool recursive)
{
    ...
    while ((ptr=readdir(dp)) != NULL)
    {
        ...
        {
            files.push_back(!absname ? ptr->d_name :
                                       join(path, ptr->d_name));
        }
